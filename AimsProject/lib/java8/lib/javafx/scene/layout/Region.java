...
public class
    ...
    private static double getSnapScale(Node n) {
        return _getSnapScale(n.getScene());
    }
    private static double _getSnapScale(Scene scene) {
    ...
    private double snapSpace(double value, boolean snapToPixel) {
        return snapToPixel ? scaledRound(value, getSnapScale()) : value;
    }
    ...
    private double snapSize(double value, boolean snapToPixel) {
        return snapToPixel ? scaledCeil(value, getSnapScale()) : value;
    }
    ...
    private double snapPortion(double value, boolean snapToPixel) {
        if (!snapToPixel || value == 0) return value;
        double s = getSnapScale();
        value *= s;
        if (value > 0) {
    ...
    static double getAreaBaselineOffset(List<Node> children, Callback<Node, Insets> margins,
        ...
        for (int i = 0;i < children.size(); ++i) {
            ...
            if (snapToPixel && i == 0) snapScaleV = getSnapScale(n.getParent());
    ...
    */
    ...
    public static void positionInArea(Node child, double areaX, double areaY, double areaWidth, double areaHeight,
        ...
        double snapScale = isSnapToPixel ? getSnapScale(child) : 1.0;

        position(child, areaX, areaY, areaWidth, areaHeight, areaBaselineOffset,
                snapSpace(childMargin.getTop(), isSnapToPixel, snapScale),
                snapSpace(childMargin.getRight(), isSnapToPixel, snapScale),
                snapSpace(childMargin.getBottom(), isSnapToPixel, snapScale),
                snapSpace(childMargin.getLeft(), isSnapToPixel, snapScale),
    ...
    public static void layoutInArea(Node child, double areaX, double areaY,
        ...
        double top = snapSpace(childMargin.getTop(), isSnapToPixel, snapScale);
        double bottom = snapSpace(childMargin.getBottom(), isSnapToPixel, snapScale);
        double left = snapSpace(childMargin.getLeft(), isSnapToPixel, snapScale);
        double right = snapSpace(childMargin.getRight(), isSnapToPixel, snapScale);
        ...
        if (valignment == VPos.BASELINE) {
            ...
            if (bo == BASELINE_OFFSET_SAME_AS_HEIGHT) {
                if (child.isResizable()) {
                    ...
                    bottom += snapSpace(areaHeight - areaBaselineOffset, isSnapToPixel, snapScale);
                } else {
                    top = snapSpace(areaBaselineOffset - child.getLayoutBounds().getHeight(), isSnapToPixel, snapScale);
                }
            } else {
                top = snapSpace(areaBaselineOffset - bo, isSnapToPixel, snapScale);
            }
        }
        ...
        if (child.isResizable()) {
            ...
            child.resize(snapSize(size.x, isSnapToPixel, snapScale),
                         snapSize(size.y, isSnapToPixel, snapScale));
        }
        position(child, areaX, areaY, areaWidth, areaHeight, areaBaselineOffset,
    ...
    private static void position(Node child, double areaX, double areaY, double areaWidth, double areaHeight,
        ...
        if (isSnapToPixel) {
            x = snapPosition(x, true, getSnapScale(child));
            y = snapPosition(y, true, getSnapScale(child));
        }
